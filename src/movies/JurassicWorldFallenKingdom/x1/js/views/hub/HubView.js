define([
    ...
    return InteractiveComp.View.extend({
        ...
        render: function () {
            ...
            this.videoTimeUpdate = function (e) {
                if (this.currentHubSelection == 1) {
                    if (this.video[0].currentTime >= 10.2 && this.video[0].currentTime <= 13.2
                        ||
                        this.video[0].currentTime >= 99.13 && this.video[0].currentTime <= 100.13
                    ) {

                        this.video[0].currentTime = 2.2;
                    }
                } else if (this.currentHubSelection == 2) {
                    if (this.video[0].currentTime >= 21.16 && this.video[0].currentTime <= 24.16
                        ||
                        this.video[0].currentTime >= 88.17 && this.video[0].currentTime <= 89.17
                    ) {
                        this.video[0].currentTime = 13.16;
                    }
                } else if (this.currentHubSelection == 3) {
                    if (this.video[0].currentTime >= 32.08 && this.video[0].currentTime <= 35.08
                        ||
                        this.video[0].currentTime >= 77.21 && this.video[0].currentTime <= 78.21
                    ) {
                        this.video[0].currentTime = 24.12;
                    }
                } else if (this.currentHubSelection == 4) {
                    if (this.video[0].currentTime >= 43.04 && this.video[0].currentTime <= 46.04
                        ||
                        this.video[0].currentTime >= 67.01 && this.video[0].currentTime <= 68.01
                    ) {
                        this.video[0].currentTime = 35.08;
                    }
                } else if (this.currentHubSelection == 0) {
                    if (
                            (this.video[0].currentTime >= 53.04 && this.video[0].currentTime <= 56.04)
                            ||
                            (this.video[0].currentTime >= 108.35 && this.video[0].currentTime <= 109.35)
                        ) {
            ...
            this.video[0].src = this.baseUrl + this.hubVideoUrl + "JWFK_NG_ALL_ACCESS_HUB.mp4" + "?NO_MTHEORY_DECODER";
        ...
        setupTouchGestures: function () {
            ...
            Hammer($('#hub-nav-touch-area', this.el)[0]).on('tap', function(){
            ...
            _.each($('.nav-link', this.el), function(element){
        ...
        onNavSelect: function() {
            if (this.getCurrentContent().content) {
                ...
                this.OmnitureAnalyticsHelper.setAction("Sub experience " + this.getCurrentContent().name + " is Selected", true);
        ...
        onKeyDown: function (code) {
            ...
            this.handleNavigation(code === this.KEY_RIGHT ? 1 : -1);
        },
        ...
        handleNavigation: function (offset, select) {
            ...
            if (offset < 0) {
                this.updateVideoTimer("LEFT");
            } else if (offset > 0) {
                this.updateVideoTimer("RIGHT");
            }
            this.updateThumbs();
        ...
        updateVideoTimer: function (navigationDirection) {
            ...
            if (this.currentHubSelection == 0) {
                ...
                this.video[0].currentTime =  navigationDirection == "RIGHT" ? 10.7 : 107.63;
            } else if (this.currentHubSelection == 2) {
                this.video[0].currentTime =  navigationDirection == "RIGHT" ? 21.66 : 96.67;
            } else if (this.currentHubSelection == 3) {
                this.video[0].currentTime =  navigationDirection == "RIGHT" ? 32.42 : 85.71;
            } else {
                this.video[0].currentTime =  navigationDirection == "RIGHT" ? 43.58 : 75.01;
            }
        },
        ...
        navigate: function (offset, select) {
            ...
            for (var iteration = 0; iteration < iterations; iteration++) {
                var index = -1, content, scale, targetPos;
                ...
                _.delay(function () {
                    ...
                    _.each(self.thumbs, function ($thumb) {
                        ...
                        if (index === 0 && direction === 1) {
                            ...
                            self.navLinkRight.css({
                                backgroundImage: 'url(' + self.baseUrl + content.nor_image + ')',
                                transform: 'translate('
                                    + content.positions[5] + 'px, '
                                    + content.y + 'px) scale(1)'
                            })
...
/*
                            ...
                            targetPos = self
                                .contentItems[self.getIndexOffset(index)]
                    ...
                    currentIteration++;
                    index = -1, content = null, scale = null, contentLeftMargin = -200;
                }, 15 + (iteration * 500) );
            }
        },
        ...
        updateThumbs: function () {
            ...
            _.each(this.thumbs, function($thumb){
                ...
                */
                ...
                $thumb.css({
                    'position': 'absolute',
                    'transform': 'translate('+ targetPos +'px, '+ content.y +'px) scale(' + scale + ')',
                    'opacity': scale,
        ...
        resume: function () {
            this.fadeInAudioElementWithSrcAndDuration(this.audio[0], this.audioSrc, 2000, true);
