define([
  ...
  return InteractiveComp.View.extend({
    ...
    startAnalytics: function() {
      ...
      this.OmnitureAnalyticsHelper.setPage(
        'Main Menu - The Fate of the Furious Franchise Hub',
        true
      );
    },
    ...
    render: function() {
      ...
      setTimeout(function() {
        if (
          self.browserPlatformHelper.isExperienceOnEmulator() ||
          (self.app && self.app.debug)
        ) {
      ...
      this.audio[0].src =
        this.baseUrl +
        'Assets/Audio/FOTF_NextGen_audiobed.mp3?NO_MTHEORY_DECODER';
    ...
    onNavSelect: function(element, timelineBookmark) {
      ...
      if (timelineBookmark) {
        this.currentSelection = this.contentItems.findIndex(function(
          contentItem
        ) {
        ...
        this.hubNav.css(
          ...
          'url("' +
            this.baseUrl +
            this.contentItems[this.getIndexOffset(0)].sel_image +
            '")'
        );
      } else {
      ...
      if (this.subview) {
        ...
        var $selectorToFadeOut = $('#sub-view', this.el).html()
          ? $('#sub-view', this.el)
          : $('#hub-background', this.el);

        var initFn = function() {
          self.loadedViewIndex = self.currentSelection;
          self.fm.initFocus(
            $('#sub-view', self.el).attr('data-position'),
            true,
            true,
            true,
            true
        ...
        var viewLoadFn = function() {
          $('#sub-view', self.el).fadeIn(function() {
            self.subview.viewLoaded
              ? self.subview.viewLoaded(initFn)
              : initFn();
          });
        };
        ...
        var fadeOutAndSetNewView = function() {
          ...
          $selectorToFadeOut.fadeOut(function() {
            ...
            self.subview.loadAssets().done(function() {
              self.subview.onViewLoad
                ? self.subview.onViewLoad(viewLoadFn)
                : viewLoadFn();
            });
          });
        };

        $selectorToFadeOut.selector === '#sub-view' &&
        previousSubView.unloadView &&
        _.isFunction(previousSubView.unloadView)
    ...
    removeSubView: function() {
      ...
      $subView.fadeOut(function() {
        ...
        $hubBackground.fadeIn(function() {
          self.fm.initFocus(
            $('#hub-nav', self.el).attr('data-position'),
            true,
            true,
            true,
            true
    ...
    onKeyDown: function(code) {
      ...
      if (
        this.subview &&
        !this.subview.$el.attr('data-disabled') &&
        code == this.KEY_UP
      ) {
        this.hubNav.removeClass('blink');

        this.currentSelection =
          this.loadedViewIndex ||
          (this.loadedViewIndex === 0 ? 0 : this.currentSelection);
        this.updateThumbs();
        this.updateContentText();
      ...
      $('.nav-link').bind('webkitTransitionEnd transitionend', function() {
        self.hubNav.css(
          ...
          'url("' +
            self.baseUrl +
            self.contentItems[self.getIndexOffset(0)].sel_image +
            '")'
        );
        self.hubNav.show();
    ...
    navigateLeft: function() {
      ...
      setTimeout(function() {
        _.each(navLinks, function($navLink) {
          ...
          $navLink === self.navLinkLeft ? $navLink.fadeIn() : null;
      ...
      setTimeout(function() {
        _.each(navLinks, function($navLink) {
          ...
          $navLink === self.navLink1
            ? $navLink
                .addClass('fadeOut')
                .css('transform', 'translateX(-360px)')
    ...
    updateThumbs: function() {
      ...
      _.each(thumbs, function($thumb) {
        index =
          $thumb === self.navLinkLeft
            ? 5
            : $thumb === self.navLinkRight
              ? 0
              : i++;

        img =
          self.baseUrl +
          self.contentItems[self.getIndexOffset(index)].nor_image;
    ...
    updateContentText: function() {
      ...
      if (this.getCurrentContent().content_text) {
        this.contentText.css(
          'background-image',
          'url("' + this.baseUrl + this.getCurrentContent().content_text + '")'
        );
    ...
    onBackButtonPressed: function() {
      if (
        (this.loadedViewIndex || this.loadedViewIndex === 0) &&
        this.subview &&
    ...
    onActiveElementChange: function(
      trigger,
      element,
      hitEnter,
      previousActive
      ...
      if (
        ...
        if (
          ...
          _.isFunction(this.subview.onBlurToSuperView)
        ) {
          this.subview.onBlurToSuperView();
        }
      }
    ...
    getIndexOffset: function(offset) {
      ...
      if (offset < 0 && -1 * offset <= this.contentItems.length) {
        ...
        return (
          (this.currentSelection + this.contentItems.length + offset) %
