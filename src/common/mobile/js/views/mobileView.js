...
define(['react', 'reactDOM', 'jquery', 'underscore', 'backbone', 'hammer', 'common/js/utils/preloader', 'common/js/utils/OmnitureAnalyticsHelper', 'orientationHelper', 'mobilelib/components/HamburgerMenu/HamburgerMenu', 'text!experience/hub.json', 'common/js/utils/CONSTANTS', 'common/mobile/js/components/HamburgerMenu/HAMBURGER_MENU_STATE_CONSTANTS'], function (React, ReactDOM, $, _, Backbone, Hammer, Preloader, OmnitureAnalyticsHelper, OrientationHelper, HamburgerMenu, MenuItems, CONSTANT, HAMBURGER_MENU_STATE) {
    ...
    return Backbone.View.extend({
        ...
        initialize: function initialize(options) {
            ...
            if (options && !options.noAnalytics) {
                this.OmnitureAnalyticsHelper = new OmnitureAnalyticsHelper({ titleMetaData: this.app.movieMetadata }, options && options.experienceName ? options.experienceName : null);
            }
            ...
            this.handlesOrientations = options.metadata.mobile.handles_orientations ? options.metadata.mobile.handles_orientations.split(',') : ["portrait"];
        ...
        render: function render() {
            ...
            if (this.hasHamburgerMenu) {
                ReactDOM.render(React.createElement(HamburgerMenu, {
                    router: this.app,
                    menuItems: this.hamburgerMenuItems,
                    ref: function ref(comp) {
                        _this.HamburgerMenu = comp;
                    }
        ...
        showLoaders: function showLoaders(show) {
            if (show) {
                if (!this.playsIntroVideo) {
                    this.startExperience.show();
                }
        ...
        setBackgroundUrl: function setBackgroundUrl(url_16x9, url_3x2) {
            var aspectRatio = OrientationHelper.determineAspectRatio(window.screen.width, window.screen.height);
        ...
        handleOrientationChange: function handleOrientationChange() {
            var supportedOrientation = this.handlesOrientations.find(function (orientation) {
        ...
        /* Helper Methods */
        getTargetByMobileViewName: function getTargetByMobileViewName(mobileViewName) {
            return _.findWhere(this.app.movieMetadata.content, { mobile_view_name: mobileViewName });
        },

        getTargetByName: function getTargetByName(name) {
