...
define([
    ...
    'common/mobile/js/components/VideoPlayer/VideoPlayer'
], function (React, ReactDOM, $, _, Backbone, Hammer, OrientationHelper, MobileView, VideoPlayer) {
    return MobileView.extend({
        ...
        setupTouchGestures: function () {
            ...
            Hammer(this.videoTimeline).on('tap', function(event){
                ...
                let percentageOnBar = (event.center.x - this.videoTimeline.offsetLeft) / this.videoTimeline.clientWidth;
                let seconds = Math.floor(this.videoDuration * percentageOnBar);
            ...
            Hammer(this.videoTimelineThumb).on('panstart', function(){
                ...
                let initialPosition = this.videoTimeline.offsetLeft - this.videoTimelineThumb.clientWidth/2;
            ...
            Hammer(this.videoTimelineThumb).on('panleft panright', function(event){
                let currentPosition = ((this.dragStartPosition + event.deltaX)/this.videoTimeline.clientWidth) * this.videoElement.duration;
        ...
        resetSource: function(url, in_time, out_time) {
            let self = this;
            ...
            let video = this.$('#video-plane', this.el);
        ...
        playVideo: function(time) {
            ...
            let video = $('#video-plane', this.el)[0];
        ...
        updateTimelineThumb: function(currentTime, duration) {
            ...
            let dx = (currentTime / duration) * this.videoTimeline.clientWidth;
            $('#video-timeline-thumb').css({'-webkit-transform': 'translateX(' + dx + 'px)'});
            $('#video-completed').css('width', dx + 'px');

            let min = Math.floor(currentTime / 60).toString(10);
            let sec = Math.floor(currentTime % 60).toString(10);
            if (sec < 10) {
                sec = "0" + sec;
            }
        ...
        clearEventHandlers: function() {
            let video = this.$('#video-plane', this.el)[0];
